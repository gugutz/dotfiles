...
(setq next-line-add-newlines nil) ;; C-n insert newlines if the point is at the end of the buffer.
...
(setq auto-save-list-file-prefix (concat user-emacs-directory "cache/auto-save-list"))
...
(use-package recentf
  ...
  :config
  (setq recentf-save-file (concat user-emacs-directory "config/recentf"))
...
(use-package amx
  :after ivy
  :config
  (setq amx-save-file (concat user-emacs-directory "config/amx-items"))
  )
...
(use-package saveplace
  ...
  (setq save-place-file (concat user-emacs-directory "config/places"))
...
;; FIXME: doom emacs recomends add-to-list instead of set-frame-font for being faster, but it was only changing the font of the childframes, not the main one.
...
(use-package projectile
  ...
  (setq projectile-known-projects-file (concat user-emacs-directory "config/projectile-bookmarks.eld")) ;;  NOTE: remove this
...
(use-package lsp-mode
  ...
  (setq lsp-session-file (concat user-emacs-directory "config/lsp-session-v1"))
  ...
  (with-eval-after-load 'evil
    ...
    (define-key evil-normal-state-map (kbd "SPC l f d")  'lsp-find-definition)
    (define-key evil-normal-state-map (kbd "SPC l g i")  'lsp-goto-implementation)
    (define-key evil-normal-state-map (kbd "SPC l f i") 'lsp-find-implementation)
    (define-key evil-normal-state-map (kbd "SPC l d") 'lsp-ui-doc-show)
    (define-key evil-normal-state-map (kbd "SPC l m") 'lsp-ui-imenu)
    (define-key evil-normal-state-map (kbd "SPC l c") 'lsp-execute-code-action)
...
(use-package lsp-ui
  ...
  ;;
  ;; lsp-ui-sideline
  ;; using the sideline only to show code actions, not diagnostics
...
(use-package flycheck
  ...
  :custom
  (flycheck-check-syntax-automatically '(save idle-change mode-enabled newline))
...
(use-package shell-pop
  ...
  ("C-c s" . shell-pop)
  :hook
  (evil-mode . (lambda() (define-key evil-normal-state-map (kbd "SPC !") 'shell-pop)))
  )
...
(defun find-css-counterpart (file)
...
(defun css-open-counterpart ()
  "Opens the corresponding template or component file to this one."
  (interactive)
  (find-file (find-css-counterpart (buffer-file-name))))

(defun find-ts-counterpart (file)
  "Return the file name of FILE's counterpart, or FILE if there is no counterpart."
  (let ((ext (file-name-extension buffer-file-name))
         (base (file-name-sans-extension buffer-file-name)))
    (concat base ".ts")))

(defun open-ts-counterpart ()
  "Opens the corresponding template or component file to this one."
  (interactive)
  (find-file (open-css-counterpart (buffer-file-name))))

(defun open-html-counterpart (file)
  "Return the file name of FILE's counterpart, or FILE if there is no counterpart."
  (let ((ext (file-name-extension buffer-file-name))
         (base (file-name-sans-extension buffer-file-name)))
    (concat base ".html")))

(defun open-ts-counterpart (file)
  "Return the file name of FILE's counterpart, or FILE if there is no counterpart."
